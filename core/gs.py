...
class GaussianRenderer:
    ...
    def render(self, gaussians, cam_view, cam_view_proj, cam_pos, bg_color=None, scale_modifier=1):
        ...
        for b in range(B):
            ...
            rgbs = gaussians[b, :, 11:].contiguous().float() # [N, 4]
        ...
        images = torch.stack(images, dim=0).view(B, V, 3, self.opt.output_size, self.opt.output_size) # we use 4 for latent
        alphas = torch.stack(alphas, dim=0).view(B, V, 1, self.opt.output_size, self.opt.output_size)

        return {
            "image": images, # [B, V, 4, H, W]
