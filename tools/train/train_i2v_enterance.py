...
def train_i2v_entrance(cfg_update,  **kwargs):
...
def worker(gpu, cfg):
    ...
    if cfg.UNet.use_lgm_refine:
        model.resume_lgm(cfg.lgm_pretrain) # initialize lgm for image to multi-view generation
    ...
    if cfg.UNet.use_lgm_refine:
        os.system("rm ./trainable_params_i2v.txt")
        for k,v in model.named_parameters():
            if v.requires_grad:
                with open('trainable_params_i2v.txt','a',encoding='utf8') as file:
                    file.write(k+"\n")
    ...
    for step in range(resume_step, cfg.num_steps + 1): 
        ...
        if cfg.vid_dataset['prepare_lgm']: # 
            gs_data, ref_frame, vit_frame, video_data, camera_data, mask_data, captions, video_key = batch
        else:
            ref_frame, vit_frame, video_data, camera_data, mask_data, captions, video_key = batch
            gs_data = None
        ...
        # process ref frame
        with torch.no_grad():
            local_image = autoencoder.encode_firsr_stage(ref_frame, cfg.scale_factor).detach()
            local_image = local_image.unsqueeze(2).repeat_interleave(repeats=cfg.max_frames, dim=2)

        # preprocess
        with torch.no_grad():
            y_visual, y_text, y_words = clip_encoder(text=captions, image=vit_frame) # bs * 1 *1024 [B, 1, 1024]
        ...
        # forward
        # model_kwargs = {'y': y_words,  'fps': fps_tensor, 'camera_data': camera_data, 'gs_data': gs_data}
        model_kwargs = {'y': y_words, 'image':y_visual, 'local_image':local_image, 'fps': fps_tensor, 'camera_data': camera_data, 'gs_data': gs_data}

        if cfg.use_fsdp:
            loss = diffusion.loss(x0=video_data,
                t=t_round, step=step, rank=cfg.rank, model=model, autoencoder=autoencoder, model_kwargs=model_kwargs,
                ...
                visual_kwards = [
                    {
                        ...
                        'image':y_visual[:viz_num], 
                        'local_image':local_image[:viz_num],
